...
The key parameter of interest is the conditional variance, $\sigma_{it}^2$. The one-period-ahead volatility forecasts, $\sigma_{it}$, are larger when $\epsilon_{i,t-1}^2$ and $\sigma_{i,t-1}^2$ are larger. In other words, the model predicts that large shocks will be followed by other large shocks. \textcolor{red}{As robustness checks, we also estimate volatility using exponential GARCH, threshold GARCH, fractionally integrated GARCH, and GARCH(1,1) accounting for structural breaks in the time series data in the appendix}. 
